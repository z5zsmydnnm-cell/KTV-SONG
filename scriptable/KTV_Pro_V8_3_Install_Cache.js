// KTV Pro V8.3 Personal - install/update local cache
// Run this manually when you want to install or update KTV Pro on this iPhone.
// The daily loader never downloads from GitHub.
...
try {
  ...
  writeStatus("install-error", String(e));
  const a = new Alert();
  a.title = "KTV Cache Install Error";
  a.message = String(e);
  a.addAction("OK");
  await a.presentAlert();
}
...
async function downloadScript() {
  ...
  if (!code || String(code).indexOf(SCRIPT_MARKER) < 0) {
    throw new Error("Downloaded script content is not valid.");
  }
  return code;
}
...
function writeStatus(source, message) {
  ...
  writeVisibleFile(STATUS_FILE_NAME, text);
}
